...
import { REDUCED_QUALITY, THUMBNAIL_SIZE } from "./UploadService";
...
export class RotateService {
  ...
  private async _renderParts(original: Blob, rotation: Rotation) {
    ...
    const reduced = await this._canvasService.resize(canvas, {
      quality: REDUCED_QUALITY,
    });
    const thumbnail = await this._canvasService.resize(canvas, {
