...
- `/bin/:albumId` → `pages/Album` — upload, view, download. Its `Container` is `position: relative` with `paddingBottom: FOOTER_HEIGHT` so `components/Footer.tsx` (absolute, bottom 0, 4rem) sits at the page bottom even for an empty album; the fixed `SelectionBar` floats above it while a selection is active. The footer's only content is "Delete this album", which opens `components/DeleteAlbumDialog.tsx` (on `components/Dialog.tsx`, a native `<dialog>` with `showModal()`, Escape/backdrop close): the red button is enabled only when the input equals the album title exactly, or `DELETE_FALLBACK_PHRASE` for an untitled album (`utils/deleteConfirmation.ts: requiredPhrase`, `confirmsDeletion`, vitest). Confirming calls `deleteAlbum` (server `rm -rf` of the album dir, idempotent; anyone with the link may), forgets the visited record, navigates home and toasts.
